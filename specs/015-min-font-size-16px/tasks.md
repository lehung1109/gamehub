...
- [X] T001 Initialize git worktree or branch workspace for `015-min-font-size-16px` isolation per Constitution Principle VI
...
- [X] T002 Verify baseline test suite, TypeScript compilation, and linting in `package.json`
...
- [X] T003 Spawn review subagent to verify Phase 1 setup integrity
...
- [X] T004 Start a dedicated subagent for Phase 2 execution
  - **Action**: Launch dedicated subagent context for Phase 2 tasks.
  - **Verification**: Dedicated subagent initialized.

- [X] T005 [P] Write failing unit tests for typography theme tokens in `tests/unit/typography-theme.test.ts`
...
- [X] T006 Implement font size scale overrides and base rules in `src/app/globals.css`
...
- [X] T007 Spawn review subagent to conduct code review and bug hunting for Phase 2
...
- [X] T009 Commit Phase 2 changes with descriptive conventional commit message
...
- [X] T010 Start a dedicated subagent for Phase 3 execution
  - **Action**: Launch dedicated subagent context for Phase 3.
  - **Verification**: Dedicated subagent initialized.

- [X] T011 [P] [US3] Write failing unit tests for UI component sizing in `tests/components/ui-components-typography.test.tsx`
...
- [X] T012 [P] [US3] Update `Button` component small variant in `src/components/ui/button.tsx`
...
- [X] T013 [P] [US3] Update `Toggle` component small variant in `src/components/ui/toggle.tsx`
...
- [X] T014 [P] [US3] Update `Badge` and `Tooltip` components in `src/components/ui/badge.tsx` and `src/components/ui/tooltip.tsx`
...
- [X] T015 Spawn review subagent to conduct code review and bug hunting for Phase 3
  - **Action**: Review all modified files in `src/components/ui/`.
  - **Verification**: Review subagent confirms zero bugs.

- [X] T016 Resolve any issues found by review subagent, and iteratively spawn re-review subagent until zero bugs
  - **Action**: Fix any identified issues.
  - **Verification**: Zero remaining issues.

- [X] T017 Commit Phase 3 changes with descriptive conventional commit message
...
- [X] T018 Start a dedicated subagent for Phase 4 execution
  - **Action**: Launch dedicated subagent context for Phase 4.
  - **Verification**: Dedicated subagent initialized.

- [X] T019 [P] [US1] Write failing unit tests for student badge and game typography in `tests/components/student-game-typography.test.tsx`
...
- [X] T020 [P] [US1] Update `StudentProfileBadge` in `src/components/StudentProfileBadge.tsx`
...
- [X] T021 [P] [US1] Update `StudentBadge` in `src/components/student/StudentBadge.tsx`
...
- [X] T022 [P] [US1] Update Workplace Tenses components in `src/components/tenses/TenseLessonContainer.tsx` and `src/components/tenses/QuickRulesTab.tsx`
  - **Action**:
...
- [X] T023 [P] [US1] Update Numbers & Colors game in `src/app/games/numbers-colors/page.tsx`
...
- [X] T024 Spawn review subagent to conduct code review and bug hunting for Phase 4
  - **Action**: Run review subagent across all updated game and student badge files.
  - **Verification**: Review subagent confirms zero bugs.

- [X] T025 Resolve any issues found by review subagent, and iteratively spawn re-review subagent until zero bugs
  - **Action**: Address any review feedback.
  - **Verification**: Zero remaining issues.

- [X] T026 Commit Phase 4 changes with descriptive conventional commit message
...
- [X] T027 Start a dedicated subagent for Phase 5 execution
  - **Action**: Launch dedicated subagent context for Phase 5.
  - **Verification**: Dedicated subagent initialized.

- [X] T028 [P] [US2] Write unit tests for admin layout and config forms typography in `tests/app/admin/admin-typography.test.tsx`
  - **Action**: Verify that admin data tables, form inputs, help texts, and badges render cleanly with 16px base font without clipped inputs.
  - **Verification**: Run `npm run test:run -- tests/app/admin/admin-typography.test.tsx`.

- [X] T029 [US2] Verify and fine-tune admin form helper text and table cells in `src/components/config/*.tsx` and `src/components/dashboard/*.tsx`
  - **Action**: Inspect admin tables, dialogs, badges, and action dropdowns to confirm no sub-16px inline styles or classes remain.
  - **Verification**: Clean visual alignment and no text clipping.

- [X] T030 Spawn review subagent to conduct code review and bug hunting for Phase 5
  - **Action**: Review admin and dashboard components for typography hierarchy and contrast.
  - **Verification**: Review subagent confirms zero bugs.

- [X] T031 Resolve any issues found by review subagent, and iteratively spawn re-review subagent until zero bugs
  - **Action**: Fix any identified issues.
  - **Verification**: Zero remaining issues.

- [X] T032 Commit Phase 5 changes with descriptive conventional commit message
  - **Action**: `git commit -m "feat(admin): verify and ensure admin forms and dashboard meet minimum 16px font size"`
  - **Verification**: `git log -1` confirms commit created.
...
- [X] T033 [P] Run static invariant regex check for sub-16px classes across entire `src/` directory
  - **Action**: Execute:
    ```powershell
...
- [X] T037 Spawn final feature-level review subagent for holistic review and bug hunt
...
- [X] T038 Resolve any remaining issues found in feature-level review, and iteratively re-review until zero bugs
...
- [X] T039 Create final comprehensive feature commit and push to remote
