...
describe('Analytics - Helper Functions (Filter, Search, Sort)', () => {
  ...
  it('correctly maps all game types to their Vietnamese labels', () => {
    ...
    expect(getGameLabel('wordle')).toBe('Thử Thách Đoán Từ')
    expect(getGameLabel('word-connect')).toBe('Vòng Xoay Nối Chữ')
    expect(getGameLabel('odd-one-out')).toBe('Truy Tìm Kẻ Lạc Loài')
