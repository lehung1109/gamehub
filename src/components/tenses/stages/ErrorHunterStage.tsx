...
export function ErrorHunterStage({
  ...
  const isErrorTokenChosen =
    selectedTokenIndex !== null && selectedTokenIndex === currentItem.errorTokenIndex;
  ...
  const handleTokenClick = (index: number) => {
    ...
    if (index === currentItem.errorTokenIndex) {
      setNonErrorNotice(null);
    } else {
      setSelectedReplacement(null);
      setNonErrorNotice(
        `Vị trí này không có lỗi. Từ "${currentItem.tokens[index]}" đã đúng ngữ pháp. Hãy tìm vị trí từ khác bị sai!`
      );
    }
  };
            ...
            {!isSubmitted ? (
              <div className="p-4 sm:p-6 rounded-xl bg-slate-50/80 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800">
                <div className="flex flex-wrap items-center gap-2 sm:gap-2.5">
                  {currentItem.tokens.map((token, index) => {
                    const isSelected = selectedTokenIndex === index;
                    const isTargetError = index === currentItem.errorTokenIndex;
              ...
              <div className="p-4 sm:p-6 rounded-xl bg-indigo-50/40 dark:bg-indigo-950/30 border border-indigo-100 dark:border-indigo-900/40">
                <p className="text-base sm:text-lg md:text-xl font-medium leading-relaxed text-foreground">
                  {currentItem.fullCorrectSentence}
                </p>
                ...
                <div className="mt-3 flex items-center justify-between text-xs text-muted-foreground pt-3 border-t border-indigo-100/80 dark:border-indigo-900/40">
                  <span className="font-semibold text-emerald-700 dark:text-emerald-300">
                    Từ đúng: <strong className="font-mono">{currentItem.correctToken}</strong> (thay thế cho <span className="line-through text-rose-600 dark:text-rose-400">{currentItem.tokens[currentItem.errorTokenIndex]}</span>)
                  </span>
                  ...
                  {isSupported && (
                    <Button
                      ...
                      variant="ghost"
                      size="sm"
                      onClick={() => speak(currentItem.fullCorrectSentence)}
          ...
          {isErrorTokenChosen && !isSubmitted && (
            <div className="space-y-3 pt-2 animate-in fade-in-50 duration-200">
              <span className="block text-xs font-bold text-muted-foreground uppercase tracking-wider">
                Bước 2: Chọn phương án sửa đúng cho từ &quot;{currentItem.tokens[selectedTokenIndex]}&quot;
              </span>
              ...
              <div className="flex items-center justify-between">
                ...
                {!isCorrect && (
                  <div className="text-xs sm:text-sm font-bold text-foreground">
                    Sửa đúng là: <span className="font-mono text-emerald-700 dark:text-emerald-400 underline">{currentItem.correctToken}</span>
                  </div>
                )}
              </div>
              ...
              <div className="pt-2 border-t border-border/40 space-y-2 text-xs sm:text-sm leading-relaxed">
                <div className="flex items-start gap-2">
                  ...
                  <div>
                    ...
                    <span>{currentItem.explanation.whyWrongVi}</span>
                  </div>
                </div>
                ...
                <div className="flex items-start gap-2 pl-0.5">
                  ...
                  <div className="text-muted-foreground">
                    ...
                    <span>{currentItem.explanation.workplaceImpactVi}</span>
                  </div>
                </div>
              </div>
            </div>
