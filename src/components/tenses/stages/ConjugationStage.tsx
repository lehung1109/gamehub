...
export function ConjugationStage({
  ...
  const contextMeta = CONTEXT_CONFIG[currentItem.contextType] || CONTEXT_CONFIG.email;
  const ContextIcon = contextMeta.icon;

  const fullSentence = `${currentItem.textBefore}${
    isSubmitted ? currentItem.correctAnswer : currentItem.baseVerb
  }${currentItem.textAfter}`;

  const handleSubmit = () => {
    if (!selectedAnswer.trim() || isSubmitted) return;

    const correct = isConjugationAnswerCorrect(selectedAnswer, currentItem);
  ...
  const handleNext = () => {
    if (currentIndex < total - 1) {
      ...
      // Last question finished - pass current accumulated score
          ...
          <div className="space-y-1 text-xs sm:text-sm">
            {currentItem.subject && (
              <div className="flex items-baseline gap-2">
                ...
                <span className="font-semibold text-foreground">{currentItem.subject}</span>
              </div>
            )}
            {currentItem.sender && (
              <div className="flex items-baseline gap-2">
                <span className="font-bold text-muted-foreground min-w-[60px]">From:</span>
                <span className="text-muted-foreground">{currentItem.sender}</span>
              </div>
            )}
            {currentItem.recipient && (
              <div className="flex items-baseline gap-2">
                <span className="font-bold text-muted-foreground min-w-[60px]">To:</span>
                <span className="text-muted-foreground">{currentItem.recipient}</span>
              </div>
            )}
          </div>
        </div>
        ...
        <CardContent className="p-5 sm:p-7 space-y-6">
          ...
          <div className="p-4 sm:p-6 rounded-xl bg-indigo-50/40 dark:bg-indigo-950/30 border border-indigo-100 dark:border-indigo-900/40">
            <p className="text-base sm:text-lg md:text-xl font-medium leading-relaxed text-foreground">
              <span>{currentItem.textBefore}</span>
              <span
                className={cn(
              ...
              >
                {isSubmitted
                  ? selectedAnswer || "___"
                  : selectedAnswer || `(${currentItem.baseVerb})`}
              </span>
              <span>{currentItem.textAfter}</span>
            </p>
            ...
            <div className="mt-3 flex items-center justify-between text-xs text-muted-foreground pt-3 border-t border-indigo-100/80 dark:border-indigo-900/40">
              <span className="font-semibold text-indigo-700 dark:text-indigo-300">
                Động từ nguyên thể: <strong className="font-mono">({currentItem.baseVerb})</strong>
              </span>
              ...
              <div className="flex items-center justify-between">
                ...
                {!isCorrect && (
                  <div className="text-xs sm:text-sm font-bold text-foreground">
                    Đáp án đúng: <span className="font-mono text-emerald-700 dark:text-emerald-400 underline">{currentItem.correctAnswer}</span>
                  </div>
                )}
              </div>
              ...
              <div className="pt-2 border-t border-border/40 space-y-1.5 text-xs sm:text-sm leading-relaxed">
                <div className="flex items-start gap-2">
                  ...
                  <div>
                    ...
                    <span>{currentItem.explanation.ruleVi}</span>
                  </div>
                </div>
                <div className="pl-6 text-muted-foreground">
                  {currentItem.explanation.detailedAnalysisVi}
                </div>
              </div>
            </div>
          )}
