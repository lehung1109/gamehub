...
export interface SentenceBuilderQuestionUIProps {
...
export function SentenceBuilderQuestionUI({
  item,
  currentIndex,
  total,
  score,
  ...
  const handleSubmit = () => {
    if (placedTokens.length === 0 || isSubmitted || !item) return;

    const tokenTexts = placedTokens.map((t) => t.text);
    const correct = isSentenceOrderCorrect(tokenTexts, item.correctTokenOrder);

    setIsCorrect(correct);
    setIsSubmitted(true);

    if (correct) {
      speak(item.fullSentenceEn);
    }
  };
                ...
                <div className="flex items-center justify-between">
                  ...
                  {isSupported && (
                    <Button
                      ...
                      variant="ghost"
                      size="sm"
                      onClick={() => speak(item.fullSentenceEn)}
                  ...
                  <div className="flex items-start gap-2">
                    ...
                    <div>
                      <strong className="font-bold text-foreground">
                        Mẹo ngữ pháp ({item.grammarTip.titleVi}):{" "}
                      </strong>
                      <span>{item.grammarTip.tipVi}</span>
                    </div>
                  </div>
                </div>
              </div>
