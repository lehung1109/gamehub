// src/components/space/SpaceCompendiumModal.tsx
'use client'

import React from 'react'
import { X, BookOpen, Volume2, CheckCircle2, Lock, Sparkles } from 'lucide-react'
import { getAllMissions } from '@/lib/phonics-space-engine'
import { useSpeech } from '@/hooks/useSpeech'

interface SpaceCompendiumModalProps {
  completedMissionIds: string[]
  onClose: () => void
}

export function SpaceCompendiumModal({
  completedMissionIds,
  onClose,
}: SpaceCompendiumModalProps) {
  ...
  return (
    <div
      role="dialog"
      aria-modal="true"
      aria-label="Bách khoa thiên văn không gian vũ trụ"
      ...
      <div className="relative w-full max-w-4xl max-h-[88vh] overflow-y-auto rounded-3xl bg-slate-900 border-4 border-cyan-400 p-6 sm:p-8 text-white shadow-2xl space-y-6">
        ...
        <div className="flex items-center justify-between border-b border-slate-800 pb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl sm:text-2xl font-black text-cyan-400">
                Bách Khoa Thiên Văn Vũ Trụ
              </h2>
              <p className="text-base text-slate-300 font-medium">
                Hồ sơ tinh tú & quy tắc ngữ âm tiếng Anh ({completedCount}/12 nhiệm vụ)
              </p>
            </div>
          </div>
          ...
          <button
            type="button"
            onClick={onClose}
            aria-label="Đóng bách khoa thiên văn"
                ...
                >
                  ...
                  <div className="space-y-1">
                    <h3 className="text-lg font-bold text-slate-400">
                      Tín Hiệu Vũ Trụ Chưa Rõ ({mission.sector.toUpperCase()})
                    </h3>
                    <p className="text-base text-slate-500">
                      Hãy lái phi thuyền đến phân khu và giải mã tín hiệu để mở khóa hồ sơ này!
                    </p>
                  </div>
                </div>
              )
            ...
            return (
              ...
              >
                <div className="flex items-start justify-between gap-3">
                  ...
                  <button
                    type="button"
                    onClick={() => handlePronounce(mission.nameEn)}
