...
import type { AnyGameSettings, GameConfig, GameId } from '@/types/config'
import { getDefaultSettings } from '@/lib/game-config-schema'
import { updateConfig } from '@/app/actions/configs'
...
export function ConfigEditForm({ game, config }: Props) {
  const router = useRouter()
  const gameId = game.id as GameId
  const [name, setName] = useState(config.name || '')
  const [settings, setSettings] = useState<AnyGameSettings>(() => {
    return (config.settings as unknown as AnyGameSettings) || getDefaultSettings(gameId)
  })
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!trimmed) {
      setErrorMessage('Tên cấu hình không được để trống')
      return
    }
    ...
    startTransition(async () => {
      try {
        const res = await updateConfig(config.id, {
          name: trimmed,
      ...
      <Card className="border-slate-200 bg-white">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <CardTitle className="text-xl font-bold text-slate-900">
                Chỉnh sửa cấu hình: {config.name}
              </CardTitle>
              <CardDescription className="text-xs text-slate-500">
                Trò chơi: {game.titleVi} • ID: {config.id}
              </CardDescription>
            </div>
          </div>
        </CardHeader>
        ...
        <CardContent className="space-y-6">
          ...
          <div className="space-y-2">
            ...
            <Input
              id="configName"
              placeholder="Ví dụ: Lớp 1A - Bài học tuần 3..."
              value={name}
            ...
            <p className="text-xs text-slate-500">
              Cập nhật tên để phản ánh nội dung đã điều chỉnh cho học sinh.
            </p>
          </div>
